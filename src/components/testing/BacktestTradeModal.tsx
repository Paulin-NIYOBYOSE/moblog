...
import type { BacktestTrade, BacktestTradeInput, Session } from "@/lib/testing/types";
...
export default function BacktestTradeModal({
  open,
  sessions,
  trade,
  defaultSessionId,
  ...
  const sessionId = form.sessionId || defaultSessionId || sessions[0]?.id || "";
  ...
  useEffect(() => {
    ...
    if (trade) {
      ...
      const session = sessions.find((s) => s.id === (defaultSessionId || sessions[0]?.id));
      setForm({
        ...EMPTY,
  ...
  }, [open, trade, defaultSessionId, sessions]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    const activeSession = form.sessionId || defaultSessionId || sessions[0]?.id;
    if (!activeSession) {
    ...
    setSaving(true);
    try {
      const input: BacktestTradeInput = {
          ...
          >
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              <h2 className="text-base font-semibold">
                {isEdit ? "Edit backtest trade" : "Add backtest trade"}
              </h2>
              <button
                type="button"
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-4">
              ...
              {showMore && (
                <div className="mt-2 grid animate-fade-in grid-cols-1 gap-3 sm:grid-cols-2">
                  <label className="block">
