...
// PUT /api/backtest-trades/:id -> update
export async function PUT(request: Request, { params }: Params) {
  ...
  try {
    ...
    if (body.sessionId !== undefined) data.sessionId = String(body.sessionId).trim();
    ...
    const trade = await prisma.backtestTrade.update({ where: { id }, data });
    return NextResponse.json(trade);
  } catch (error) {
    console.error("PUT /api/backtest-trades/:id failed:", error);
    return NextResponse.json({ error: "Failed to update trade." }, { status: 500 });
  }
}

// DELETE /api/backtest-trades/:id
export async function DELETE(_request: Request, { params }: Params) {
  ...
  try {
    const { id } = await params;
    await prisma.backtestTrade.delete({ where: { id } });
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("DELETE /api/backtest-trades/:id failed:", error);
